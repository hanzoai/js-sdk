...
const prompt = `Runs JavaScript code to interact with the Hanzo API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      client_envs: JSON.stringify({
        HANZO_API_KEY: requireValue(
          readEnv('HANZO_API_KEY') ?? client.apiKey,
          'set HANZO_API_KEY environment variable or provide apiKey client option',
        ),
        HANZO_BASE_URL:
          readEnv('HANZO_BASE_URL') ?? readEnv('HANZO_ENVIRONMENT') ? undefined : client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'hanzo-ai',
      code,
      intent,
      client_opts: { environment: (readEnv('HANZO_ENVIRONMENT') || undefined) as any },
