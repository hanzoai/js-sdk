...
export { ClientOptions } from 'hanzoai';
...
async function getInstructions() {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/hanzo-ai',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the hanzo-ai MCP server. You will use Code Mode to help the user perform
...
export async function initMcpServer(params: {
  ...
  let client = new Hanzo({
    ...{ environment: (readEnv('HANZO_ENVIRONMENT') || undefined) as any },
    logger,
