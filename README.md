...
```sh
npm install git+ssh://git@github.com:stainless-sdks/Hanzo-AI-typescript.git
```
...
const response = await client.getHome().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: response, response: raw } = await client.getHome().withResponse();
...
1. Via the `HANZO_LOG` environment variable
...
const client = new Hanzo({
  logger: logger.child({ name: 'Hanzo' }),
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/Hanzo-AI-typescript/issues) with questions, bugs, or suggestions.
